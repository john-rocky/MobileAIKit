        ...
        .task {
            if agent == nil {
                agent = await AgentKit.build(
                    ...
                    options: .init(
                        systemPrompt: """
                        You are a demo assistant that shows off on-device tool calling. \
                        Prefer tools when the user asks for fresh data, device context, or app actions. \
                        After each tool call, reply in 1–2 sentences summarising the result.
                        """
                    )
                )
            }
